...
export class ManageCoursescheduleComponent implements OnInit {
  ...
  GetCourseScheduleById() {
    debugger
    this.lmsSvc.GetCourseScheduleById(this.ScheduleId).subscribe({
      next: (res: CourseScheduleVM[]) => {
        debugger
        this.getSchById = res;
  ...
  SaveSchedule() {
    ...
    this.lmsSvc.SaveCourseSchedule(this.selectedSchedule).subscribe({
      next: (res: CourseScheduleVM) => {
  ...
  UpdateSchedule() {
    this.lmsSvc.UpdateCourseSchedule(this.selectedSchedule).subscribe({
      next: (res: CourseScheduleVM) => {
        if (this.Edit) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          if (ScheduleDet.id == undefined || ScheduleDet.id == 0) {
            ...
            this.lmsSvc.UpdateCourseSchedule(Schedule).subscribe({
