...
namespace QST.MicroERP.Controllers
{
    ...
    public class UserRoleController : ControllerBase
    {
        ...
        //[HttpPost]
        //public async Task<ActionResult> AddUsersInRoles(List<User> users, string roleName)
        //{
        ...
        //[HttpDelete]
        //public async Task<ActionResult> RemoveUsersFromRole(List<User> users, string roleName)
        //{
        //    for (int i = 0; i < users.Count; i++)
        //    {
        //        var user = await userManager.FindByIdAsync(users[i].Id);
        ...
        public async Task<ActionResult> RemoveUsersFromRole(UserRoles uroles)
        {
                ...
                var user = await userManager.FindByNameAsync(uroles.uName);
            ...
            try {
                ...
                foreach (var us in users.ToList())
                {
                   // var user = await userManager.FindByIdAsync(us.Id);
                    ...
                    uRoles.role = roles;
                    UserRoles uRole = new UserRoles { userId = us.Id, role = uRoles.role,uName=us.UserName };
                    uroles.Add(uRole);
                }
            }
            catch(Exception ex)
